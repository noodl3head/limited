...
from helmet_assistant.config import AssistantConfig
...
async def _push_to_talk(ctx: agents.JobContext) -> None:
    loop = asyncio.get_running_loop()
    pressed = False

    async def set_mic(enabled: bool) -> None:
        await ctx.room.local_participant.set_microphone_enabled(enabled)
        logger.info("PTT %s", "ON" if enabled else "OFF — processing")

    def on_press(key: kb.Key) -> None:
        nonlocal pressed
        if key == kb.Key.space and not pressed:
            pressed = True
            asyncio.run_coroutine_threadsafe(set_mic(True), loop)

    def on_release(key: kb.Key) -> None:
        nonlocal pressed
        if key == kb.Key.space and pressed:
            pressed = False
            asyncio.run_coroutine_threadsafe(set_mic(False), loop)
...
async def entrypoint(ctx: agents.JobContext) -> None:
    ...
    asyncio.create_task(_push_to_talk(ctx))
